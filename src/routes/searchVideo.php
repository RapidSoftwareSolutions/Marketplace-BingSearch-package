...
$app->post('/api/BingSearch/searchVideo', function ($request, $response, $args) {
    ...
    if(!empty($post_data['args']['videoLength'])) {
        $query['videoLength'] = $post_data['args']['videoLength'];
    }
    if(!empty($post_data['args']['resolution'])) {
        $query['resolution'] = $post_data['args']['resolution'];
    }
    
    $headers['Ocp-Apim-Subscription-Key'] = $post_data['args']['subscriptionKey'];
    
    $query_str = $settings['api_url'].'/videos/search';
